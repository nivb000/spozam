...
const NavLinks = ({ handleClick }) => (
  <div className="mt-10">
    {links.map(link => (
      <NavLink to={link.to} key={link.name} onClick={() => handleClick && handleClick()} className="flex flex-row justify-start items-center my-8 text-sm font-medium text-gray-400 hover:text-cyan-400">
...
export const Sidebar = () => {
  ...
  return <>
    <div className="md:flex hidden flex-col w-[240px] py-10 px-4 bg-[#191624]">
      <img src={logo} alt="logo" onClick={() => navigate('/')} className="w-full h-14 object-contain cursor-pointer" />
      <NavLinks />
    </div>
